...
function Countdown() {
    ...
    const categoryColors = {
        'Meeting': 'bg-primary pattern',
        'Birthday': 'bg-danger pattern',
        'Reminder': 'bg-success pattern',
    };
    ...
    const addTimer = () => {
        ...
        const targetDateTime = new Date(newTimerDateTime).getTime(); // Convert to milliseconds since epoch

        const newTimer = {
            id: timers.length + 1,
            category: newTimerCategory,
            targetDateTime,
            timeRemaining: calculateTimeRemaining(targetDateTime), // Calculate time remaining here
            isRunning: true,
            title: newTimerTitle,
        ...
        <div className="countdown-container">
            <div className="main-container">
                <div className="input-container m-3">
                    <h1 className="text-center text-success">GeeksForGeeks Countdown Timer</h1>
                    <input
                        type="text"
                ...
                <div className="timers-div m-auto ">
                    {timers.map((timer) => {
                        const timeRemaining = formatTimeRemaining(timer.timeRemaining);

                        return (
                            <div key={timer.id} className={`card m-4 ${categoryColors[timer.category] || ''}`}>
                                ...
                                <div className="card-body d-flex">
                                    {timeRemaining.days > 0 && (
                                        <div className='container bg-light text-dark rounded m-2'>
                                            <div><h1><strong>{timeRemaining.days}</strong></h1></div>
                                            <div>days </div>
                                        </div>
                                    )}
                                    <div className='container bg-light text-dark rounded m-2'>
                                        <div><h1><strong>{timeRemaining.hours}</strong></h1></div>
                                        <div>hours </div>
                                    </div>
                                    <div className='container bg-light text-dark rounded m-2'>
                                        <div><h1><strong>{timeRemaining.minutes}</strong></h1></div>
                                        <div>minutes </div>
                                    </div>
                                    <div className='container bg-light text-dark rounded m-2'>
                                        <div><h1><strong>{timeRemaining.seconds}</strong></h1></div>
                                        <div>seconds </div>
                                    </div>
                                </div>
                                <button
